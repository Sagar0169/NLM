...
import com.udid.utilities.interactors.ObservableString
...
open class CustomBindingAdapter {
    companion object {
        ...
        fun showSnackBarb(
            ...
            Log.d("ERROR MESSAGE", snackMessageInt.toString())
            Log.d("ERROR MESSAGE2", snackMessageString.toString())
            var message = ""
            snackMessageString?.let {
                if (!TextUtils.isEmpty(it.trimmed)) {
                    message = it.trimmed
                    it.set("")
                }
            }
            snackMessageInt?.let {
                if (it.get() != 0) {
                    message = viewLayout.resources.getString(it.get())
                    it.set(0)
                }
            }
