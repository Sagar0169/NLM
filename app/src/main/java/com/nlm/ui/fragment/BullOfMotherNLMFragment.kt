...
import com.nlm.databinding.FragmentBullOfMotherNLMBinding
...
class BullOfMotherNLMFragment(
    private val viewEdit: String?,
    private val itemId: Int?,
    private val dId: Int?
) : BaseFragment<FragmentBullOfMotherNLMBinding>(), CallBackDeleteFSPAtId, CallBackBreedAvg,
    CallBackDeleteAtId, CallBackItemUploadDocEdit {
    private var mBinding: FragmentBullOfMotherNLMBinding? = null
    ...
    override val layoutId: Int
        get() = R.layout.fragment_bull_of_mother_n_l_m
    ...
    override fun init() {
        ...
        mBinding?.rgRemarksOfNlm?.setOnCheckedChangeListener { group, checkedId ->
            remarkRadio = when (checkedId) {
                R.id.rbCooperation -> {
                    mBinding?.llFarm?.showView()
                    1
                }
                R.id.rbNonCoorperation -> {
                    mBinding?.llFarm?.hideView()
                    2
                }
                R.id.rbPartialData -> {
                    mBinding?.llFarm?.showView()
        ...
        if (viewEdit == "view") {
            ...
            mBinding?.NoOfAnimals?.isEnabled = false
            mBinding?.NoOfMale?.isEnabled = false
            mBinding?.etNoOfFemaleCalves?.isEnabled = false
            ...
            mBinding?.etDisease?.isEnabled = false
            mBinding?.etEquipement?.isEnabled = false
            mBinding?.etModeOfMilking?.isEnabled = false
            mBinding?.etCommentsOfNLM?.isEnabled = false
