...
class SupportingDocumentAdapterNDD(
    private val programmeList: MutableList<Array<String>>,
) : RecyclerView.Adapter<SupportingDocumentAdapterNDD.SupportingDocument>() {
    ...
    override fun onBindViewHolder(holder: SupportingDocument, position: Int) {
        ...
        holder.binding.btnAdd.setOnClickListener {
            if(programmeList.size<5){
            programmeList.add(arrayOf(""))
            notifyItemInserted(programmeList.size - 1)
            notifyItemChanged(position)
        }else{
            Toast.makeText(holder.itemView.context, "Maximum 5 items allowed", Toast.LENGTH_SHORT).show()
            }
